...
print("INSERT INTO [scFuncionario].[Funcionario] (nome, sobrenome, dt_nascimento, cpf, rg, sexo,\
 email_id, telefone_id, contato_emergencia, endereco_id, deposito_id, conta_id, status_id)\nVALUES")
...
for var_x in range(500):
    ...
    var_dt_nascimento = str(f'{randint(1970, 2001)}-{randint(1, 12)}-{randint(1, 28)}')
    ...
    print(f"    ('{var_nome}', '{var_sobrenome}', '{var_dt_nascimento}', '{var_cpf}', '{var_rg}', '{var_sexo}', '{var_id_email}', '{var_id_telefone}', '{var_contato_emergencia}', '{var_id_endereco}', '{var_id_deposito}', {var_id_conta}, '{var_status}'){var_sep}")
